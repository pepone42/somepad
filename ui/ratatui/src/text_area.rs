...
impl StatefulWidget for TextArea {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        for line in self
            ...
            .lines()
            .skip(state.scrolly)
            .take(area.height as _)
            //.map(|l| Line::raw(get_line_info(&l,0,4)))
        {
            let line = Line::raw(get_line_info(&line,0,4));
        ...
        for s in self.sel {
            ...
            if s.head.line >= state.scrolly && s.head.line < state.scrolly + area.height as usize {
                buf.get_mut(tab2space_char_idx(&self.rope.slice(..),s.head.line as _,4)[s.head.column] as _, s.head.line as _).modifier |= Modifier::UNDERLINED;
            }
        }
    }
}
