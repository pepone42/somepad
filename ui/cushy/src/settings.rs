...
impl Default for Settings {
    fn default() -> Self {
        ...
        shortcuts.insert(
            crate::DUPLICATE_SELECTION_DOWN.id.to_string(),
            shortcut!(Ctrl + Alt + ArrowDown),
        );
        shortcuts.insert(
            crate::DUPLICATE_SELECTION_UP.id.to_string(),
        ...
        shortcuts.insert(
            crate::PREV_DOC.id.to_string(),
            shortcut!(Ctrl + Shift + Tab),
        );
        shortcuts.insert(
            crate::SHOW_ALL_COMMAND.id.to_string(),
...
impl Settings {
    fn try_load() -> anyhow::Result<Self> {
        ...
        let project_dir =
            ProjectDirs::from("rs", "", "somepad").context("Getting project config path")?;
        let config_folder = project_dir.config_dir();
        let mut theme_set = ThemeSet::load_defaults();
        theme_set
            .add_from_folder(config_folder)
        ...
        let config_file = ProjectDirs::from("rs", "", "somepad")
            .context("Getting project config path")?
            .config_dir()
            .join("settings.toml");
